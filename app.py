...
CONTRATANTES_FILE = "contratantes.json"
NOVO = "➕ Novo cadastro"


# ==================================================================
# ARMAZENAMENTO: Supabase quando configurado, JSON local como reserva
# ==================================================================
@st.cache_resource(show_spinner=False)
def conectar():
    """Devolve (cliente, mensagem). Cliente None = modo local."""
    try:
        url = st.secrets["SUPABASE_URL"]
        key = st.secrets["SUPABASE_SERVICE_KEY"]
    except Exception:
        return None, "Secrets do Supabase não configurados — usando arquivo local."
    try:
        from supabase import create_client
        return create_client(url, key), "ok"
    except Exception as e:
        return None, f"Falha ao conectar no Supabase ({e}) — usando arquivo local."
...
ARQUIVO = {"empresas_representadas": EMPRESAS_FILE, "contratantes": CONTRATANTES_FILE}


def listar(tabela, ordem="nome"):
    """Lê do banco; se falhar, usa o espelho local. Toda leitura bem-sucedida
    atualiza o espelho, então o app continua útil se o banco pausar."""
    sb, _ = conectar()
    if sb is not None:
        try:
            r = sb.table(tabela).select("*").order(ordem).execute()
            _gravar_json(ARQUIVO[tabela], r.data or [])
            return r.data or []
        except Exception as e:
            st.session_state["_erro_db"] = str(e)
    return _ler_json(ARQUIVO[tabela])


def gravar(tabela, registro, id_=None):
    """Insere ou atualiza. Devolve (ok, mensagem)."""
    sb, _ = conectar()
    if sb is not None:
        try:
            if id_:
                sb.table(tabela).update(registro).eq("id", id_).execute()
            else:
                sb.table(tabela).insert(registro).execute()
            return True, "Salvo no banco."
        except Exception as e:
            return False, f"Erro ao salvar no banco: {e}"
    # modo local
    dados = _ler_json(ARQUIVO[tabela])
    if id_:
        dados = [registro if d.get("id") == id_ else d for d in dados]
    else:
        registro = {**registro, "id": (max([d.get("id", 0) for d in dados], default=0) + 1)}
        dados.append(registro)
    _gravar_json(ARQUIVO[tabela], dados)
    return True, "Salvo localmente (sem banco configurado)."
...
def excluir(tabela, id_):
    ...
    dados = [d for d in _ler_json(ARQUIVO[tabela]) if d.get("id") != id_]
    _gravar_json(ARQUIVO[tabela], dados)
    return True, "Removido localmente."
...
def gerar_pdf(d):
    ...
    if d["modo"] == "fechado":
        ...
        for p in d["pessoas"]:
            ...
            pdf.cell(0, 4.6, _s(f"Base: {brl(d['base'])}/h  |  Extra: {brl(tf['extra_util'])}/h"
...
_sb, _msg = conectar()
if _sb is None:
    st.warning(f"⚠️ Modo local: {_msg} Cadastros ficam só nesta sessão.", icon="⚠️")
if st.session_state.get("_erro_db"):
    st.error(f"Banco indisponível ({st.session_state['_erro_db']}). "
             f"Usando a última cópia local dos cadastros.")

# valores padrão da composição (ficam em session_state para poderem ser
# sobrescritos quando um contratante é carregado)
PADRAO = {"base": 50.0, "m1": time(7, 0), "m2": time(12, 0), "t1": time(13, 0),
          "t2": time(17, 0), "pct_extra": 60.0, "pct_sabado": 60.0, "pct_domfer": 100.0,
          "cli_nome": "", "cli_email1": "", "cli_email2": ""}
for _k, _v in PADRAO.items():
    st.session_state.setdefault(_k, _v)

contratantes = listar("contratantes")
mapa_contr = {c["nome"]: c for c in contratantes}

# ---------------- SIDEBAR ----------------
with st.sidebar:
    st.header("👤 Cliente (contratante)")
    sel = st.selectbox("Selecione o cliente", [NOVO] + sorted(mapa_contr.keys()),
                       key="sel_contratante",
                       help="Ao selecionar, a composição de valores deste cliente é carregada.")

    if sel != st.session_state.get("_contr_carregado"):
        st.session_state["_contr_carregado"] = sel
        if sel != NOVO:
            c = mapa_contr[sel]
            st.session_state["base"] = float(c.get("valor_base") or 0)
            st.session_state["m1"] = to_time(c.get("manha_ini")) or time(7, 0)
            st.session_state["m2"] = to_time(c.get("manha_fim")) or time(12, 0)
            st.session_state["t1"] = to_time(c.get("tarde_ini")) or time(13, 0)
            st.session_state["t2"] = to_time(c.get("tarde_fim")) or time(17, 0)
            st.session_state["pct_extra"] = float(c.get("pct_extra") or 0)
            st.session_state["pct_sabado"] = float(c.get("pct_sabado") or 0)
            st.session_state["pct_domfer"] = float(c.get("pct_domfer") or 0)
            st.session_state["cli_nome"] = c.get("nome") or ""
            st.session_state["cli_email1"] = c.get("email1") or ""
            st.session_state["cli_email2"] = c.get("email2") or ""
            st.rerun()

    st.divider()
    st.header("⚙️ Composição de valores")
    base = st.number_input("Valor base da hora comercial (R$)", min_value=0.0, step=0.50,
                           format="%.2f", key="base",
                           help="Aceita valor quebrado, ex.: 66,50. É a única entrada do valor "
                                "da hora — toda a equipe usa este valor.")
    st.markdown("**Horário comercial (seg a sex)**")
    c1, c2 = st.columns(2)
    m1 = c1.time_input("Manhã início", step=300, key="m1")
    m2 = c2.time_input("Manhã fim", step=300, key="m2")
    c3, c4 = st.columns(2)
    t1 = c3.time_input("Tarde início", step=300, key="t1")
    t2 = c4.time_input("Tarde fim", step=300, key="t2")
    st.markdown("**Acréscimos**")
    pct_extra = st.number_input("Extra dia útil (%)", min_value=0.0, step=0.50, key="pct_extra")
    pct_sabado = st.number_input("Sábado (%)", min_value=0.0, step=0.50, key="pct_sabado")
    pct_domfer = st.number_input("Domingo / Feriado (%)", min_value=0.0, step=0.50,
                                 key="pct_domfer")

    st.divider()
    st.caption("Salvar esta composição no cadastro do cliente:")
    nome_salvar = st.text_input("Nome do cliente", key="cli_nome")
    e1 = st.text_input("E-mail 1", key="cli_email1")
    e2 = st.text_input("E-mail 2", key="cli_email2")

    reg = {"nome": nome_salvar.strip(), "email1": e1.strip(), "email2": e2.strip(),
           "valor_base": float(base), "manha_ini": m1.strftime("%H:%M"),
           "manha_fim": m2.strftime("%H:%M"), "tarde_ini": t1.strftime("%H:%M"),
           "tarde_fim": t2.strftime("%H:%M"), "pct_extra": float(pct_extra),
           "pct_sabado": float(pct_sabado), "pct_domfer": float(pct_domfer)}

    b1, b2 = st.columns(2)
    if b1.button("💾 Salvar cliente", use_container_width=True):
        if not reg["nome"]:
            st.warning("Informe o nome do cliente.")
        else:
            existente = mapa_contr.get(reg["nome"])
            ok, msg = gravar("contratantes", reg, existente["id"] if existente else None)
            (st.success if ok else st.error)(msg)
            if ok:
                st.rerun()
    if b2.button("🗑️ Excluir", use_container_width=True,
                 disabled=(sel == NOVO)):
        ok, msg = excluir("contratantes", mapa_contr[sel]["id"])
        (st.success if ok else st.error)(msg)
        if ok:
            st.session_state["_contr_carregado"] = None
            st.rerun()
...
st.caption("Esta tabela vai para o PDF com estes valores, calculados a partir dos parâmetros "
           "da barra lateral.")
...
empresas = listar("empresas_representadas")
rot = {f"{e['nome']} — {e['cnpj']}": e for e in empresas}
escolha = st.selectbox("Empresas salvas", [NOVO] + sorted(rot.keys()))
...
if escolha == NOVO:
    ...
    c3.write("")
    if c3.button("💾 Salvar", use_container_width=True):
        if empresa_nome and empresa_cnpj:
            ok, msg = gravar("empresas_representadas",
                             {"nome": empresa_nome.strip(), "cnpj": empresa_cnpj.strip()})
            (st.success if ok else st.error)(msg)
            if ok:
                st.rerun()
        else:
    ...
    empresa_nome = rot[escolha]["nome"]
    empresa_cnpj = rot[escolha]["cnpj"]
    ci, cx = st.columns([5, 1])
    ci.info(f"**{empresa_nome}**  |  CNPJ: {empresa_cnpj}")
    if cx.button("🗑️ Excluir", use_container_width=True):
        ok, msg = excluir("empresas_representadas", rot[escolha]["id"])
        (st.success if ok else st.error)(msg)
        if ok:
            st.rerun()
...
st.caption("Preenchido pelo cliente selecionado na barra lateral; dá para editar aqui "
           "sem alterar o cadastro.")
cliente_nome = st.text_input("Nome da empresa contratante",
                             value=st.session_state.get("cli_nome", ""))
ce1, ce2 = st.columns(2)
email1 = ce1.text_input("E-mail contratante 1", value=st.session_state.get("cli_email1", ""))
email2 = ce2.text_input("E-mail contratante 2", value=st.session_state.get("cli_email2", ""))
...
st.caption("Adicione quantas pessoas quiser. Todas usam o valor base e os acréscimos definidos "
           "na barra lateral (Composição de valores).")

if "equipe_base" not in st.session_state:
    st.session_state.equipe_base = pd.DataFrame([{"Nome": "", "Função": ""}])
